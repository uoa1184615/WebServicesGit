...
interactive web service~\cite[]{Roberts09c}.  Modelling
...
and/or~\verb|w(2)| are the usual choices (aliases exist for
\verb|w0| to~\verb|w9|); but other labels may be used. Often
...
the differential equations. The \sde{}s must be linearly
diagonalised. \footnote{Although a Jordan form is also
acceptable, there are issues in the error control.  Also,
`small' off-diagonal terms are allowed.}  Then, as in the
example of the next \cref{ss:eg}, the input parameters to
...
Let's make the algebra case sensitive: for \emph{psl} we set
\verb|raise|; whereas for \emph{csl} we set \verb|lower|.
Remember (currently, 2025) \verb|off latex| automatically
turns \verb|raise| back on (but not \verb|lower|).
\begin{reduce}
off raise;  off lower;
\end{reduce}
...
The code cannot handle any cubic or higher order in noise 
amplitude~\verb|sigma|.
\begin{reduce}
let sigma^3=>0;
\end{reduce}
...
Define aliases \verb|wi| to map to \verb|w(i)|.
\begin{reduce}
for i:=0:9 do if mkid(w,i) neq w(i) 
    then set(mkid(w,i),w(i));
\end{reduce}
...
\begin{reduce}
for i:=1:nx do if mkid(x,i) neq x(i) 
    then set(mkid(x,i),x(i));
\end{reduce}
...
\verb|df(,t)| on the right-hand side.
\begin{reduce}
for i:=1:ny do if mkid(y,i) neq y(i) 
    then set(mkid(y,i),y(i));
\end{reduce}
...
\verb|df(,t)| on the right-hand side.
\begin{reduce}
for i:=1:nz do if mkid(z,i) neq z(i) 
    then set(mkid(z,i),z(i));
\end{reduce}
...
$X_i$, $Y_i$, and~$Z_i$, denoted by operators \verb|X(i)|,
\verb|Y(i)| and~\verb|Z(i)|, which are nonlinear
...
depend X,t; depend Y,t; depend Z,t;
let { df(X(~i),t)=>ff(i)*cmanf 
    , df(Y(~i),t)=>gg(i)*smanf
    , df(Z(~i),t)=>hh(i)*umanf };
\end{reduce}
...
\begin{reduce}
for i:=1:nx do x_(i):=X(i)*cmanf;
for i:=1:ny do y_(i):=Y(i)*smanf;
for i:=1:nz do z_(i):=Z(i)*umanf;
for i:=1:nx do ff(i):=0;
for i:=1:ny do gg(i):=-rats_(i)*Y(i)*smanf;
for i:=1:nz do hh(i):=+ratu_(i)*Z(i)*umanf;
\end{reduce}
...
\begin{reduce}
y4y:=for i:=1:ny collect Y(i)=Y(i)*yz^rats_(i)$
z4z:=for i:=1:nz collect Z(i)=Z(i)/yz^ratu_(i)$
y4y:=append(y4y,z4z)$
\end{reduce}
...
%write ratsi:=rats_(i);
%write zint0:=zint_(res0,tt);
%write zint1:=zres_(res1,ttyz,rats_(i));
%write
...
if umanf then for i:=1:nz do write "dZ(",i,")/dt = ",hh(i);
if smanf then for i:=1:ny do write "dY(",i,")/dt = ",gg(i);
if cmanf then for i:=1:nx do write "dX(",i,")/dt = ",ff(i);
\end{reduce}
