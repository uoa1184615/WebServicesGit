...
\begin{abstract}
Provides a coded procedure to construct stochastic normal
form of a wide class of systems of non-autonomous or
stochastic differential equations (herein abbreviated by
\sde{}s).  The methodology is based upon  earlier
research~\cite[]{Cox91, Chao95, Roberts06k}.  Interpret all
\sde{}s in the Stratonovich sense so the analysis applies 
to deterministic differential equations, both non-autonomous
and autonomous. Cater for deterministic autonomous systems
by simply omitting the `noise'.  For generality, this coded
procedure now caters  for unstable modes, and for
differential equation systems with  a rational right-hand
side.  This code also underlies an interactive web
service~\cite[]{Roberts07d}.  Modelling stochastic systems
has many important applications.  The constructed stochastic
coordinate  transforms are a powerful way of disentangling
...
command \verb|in_tex "stoNormForm.tex"$|
...
\verb|manyExamples.pdf| for many examples.
\begin{verbatim}
...
to~\verb|x(nx)|, the stable fast modes by~\verb|y(1)|through
to~\verb|y(ny)|, and the unstable fast modes by~\verb|z(1)|
through to~\verb|z(nz)|. Each non-autonomous factor must be
...
may be used. Often \verb|w(.)| is a Stratonovich white
noise, a derivative of a Stratonovich Wiener process. 
Deterministic, autonomous, systems are analysed by omitting
any noise term~\verb|w()| in the differential equations. 
The \sde{}s must be linearly diagonalised.\footnote{Although
a Jordan form is also acceptable, there are issues in the
error control.}  Then, as in the example of the next
...
recover results for the specified system, and then
reinterpret the results, and errors, in terms of your
...
$y_i$, $z_i$~and~$w_i$ (and, to cater for rational function
right-hand sides, also may include some~\(\dot x_i\), \(\dot
y_i\), \text{and~\(\dot z_i\) effects).}


\paragraph{Outputs}  This procedure reports the specified
system, the embedded system it actually analyses, the number
of iterations taken, the constructed time dependent
coordinate transform from new variables~\((\Xv,\Yv,\Zv)\) to
the original~\((\xv,yv,zv)\), and the corresponding
...
\verb|stoNFreport.tex| (and \verb|stoNFreportSys.tex|).
Generate a pdf version by executing 
\verb|pdflatex stoNFreport|\,.

\item Global arrays such that \verb|x_(i)|, \verb|y_(j)|,
and~\verb|z_(k)| are respectively the coordinate transforms
\(x_i(\Xv,\Yv,\Zv,\sigma,\eps)\), 
\(y_j(\Xv,\Yv,\Zv,\sigma,\eps)\),
and~\(z_k(\Xv,\Yv,\Zv,\sigma,\eps)\).
...
`small'~\((x,y,\sigma)\) by executing the following
procedure \verb|examplenormform()|:
\begin{reduce}
procedure examplenormform;  
    stonormalform(
    {-x(1)*y(1)},
...
the problem, as it will be done for you.

Further, the procedure uses the parameter~\verb|small|, also
denoted by~\(\eps\), to control truncation in nonlinearity. 
The last parameter in the above specifies to construct the
...
\end{align*}
\begin{itemize}
\item Since these construction is based upon the subspace of
...
that the \(X_1,Y_1\)-system and the coordinate transform
...
\begin{reduce}
write "Construct a stochastic normal form (version 22 Apr 2021)"$
\end{reduce}
...
<<!*!*a2sfn:='texaeval;
  ...
  prin2t "\def\ou\big(#1,#2,#3\big){{{\rm e}^{\if#31\else#3\fi t}\star}#1\,}";
