...
The above right-hand side expressions for the time-derivatives must?? be multinomial in variables~$x_i$, $y_i$, $z_i$~and~$w_i$ (and also may include some~\(\dot x_i\), \(\dot y_i\), \text{and~\(\dot z_i\) effects).}
...
Hence this system is analysed for `small'~\(x,y,\sigma\) by executing the \text{following procedure:}
\begin{reduce}
...
First, form the substitutions needed to introduce~\verb|yz| to count the number of variables $Y_i$~and~$Z_i$ in any given term, weighted according to their rate coefficient in the \text{homological equation.}
\begin{reduce}
