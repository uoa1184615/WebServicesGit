...
\verb|examplesystem()|:
\begin{reduce}
procedure examplesystem();  
    slownonauto(
    mat(( -u1+u2-u1^2+w(1), u1-u2+u2^2+w(2) )),
    mat((1,1)),
    mat((-2)),
    mat((-1,1)),
    3 )$
...
\begin{reduce}
write "Construct a non-autonomous slow manifold (version 11 Jul 2023)"$
\end{reduce}
...
\begin{reduce}
write "
===== Inspect these printed equations.  Answers 
...
\begin{reduce}
  for j:=1:ns do begin
%if trace then write "starting slow mode j = ",j;%??
%write m1:=1*mode_(j,1);
%if trace then for k:=1:length(m1) do begin
%write mpart:=part(m1,k);
%write test:=(znon_(mpart,tt) where zrules);
%end;
%write "finished part-by-part";
