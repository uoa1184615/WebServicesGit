...
% Maybe use rlfi's pdf instead of diff?  But it does not handle pdf(x)!
...
Also, create the jacobian transpose matrix: take the 
conjugate later when used.
\begin{reduce}
matrix rhsjact(n,n);
for i:=1:n do for j:=1:n do 
    rhsjact(j,i):=df(ff(i,1),mkid(u,j));
\end{reduce}
...
\begin{reduce}
  lm_:=length(modes_(k_));
  matrix nxn_(n,n),lmxn_(lm_,n),lmxlm_(lm_,lm_);
  if trace then write "llzz_= ",llzz_; 
  llzz_:=llzz_^(-1);
  if trace then write "llzz_= ",llzz_;
  for i:=1:n do for j:=1:n do nxn_(i,j):=llzz_(i,j);
  l_invs(k_):=nxn_;
  for i:=1:lm_ do for j:=1:n do 
      lmxn_(part(modes_(k_),i),j):=llzz_(i+n,j);
  g_invs(k_):=lmxn_;
...
\begin{reduce}
  l1_invs(k_) := conj_(tp l_invs(k_));
  if trace then write "l1_invs",k_," =",l1_invs(k_);
  l2_invs(k_) := conj_(tp g_invs(k_));
  if trace then write "l2_invs",k_," =",l2_invs(k_);
  for i:=1:lm_ do for j:=1:n do 
      lmxn_(i,j):=conj_(llzz_(j,i+n));
  g1_invs(k_) := lmxn_;
  if trace then write "g1_invs",k_," =",g1_invs(k_);
  g2_invs(k_) := 
  for i:=1:lm_ do for j:=1:lm_ do 
      lmxlm_(i,j):= conj_(llzz_(j+n,i+n));
...
%: WORKING have to think of a different way of doing this.  Element by element??  matrix procedure??
\begin{reduce}
clear l_inv; operator l_inv; linear l_inv;
let l_inv(exp(~a_),exp) => l_invproc(a_/t);
...
\cite{Roberts96a}.
\begin{reduce}
resde:=-df(uu,t)+sub(manifold_(uu,n),ff);
