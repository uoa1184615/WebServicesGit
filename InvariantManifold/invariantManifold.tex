...
%system (for the moment up to nine). % do not seem to need u??
%\begin{reduce}
...
% freqm not used hereafter, only freq.
\begin{reduce}
array freq_(m);
...
for j:=1:m do aa_(j,j):=i*freq_(j);
...
\begin{reduce}
itisSlowMan_:=if freq_set={0} then 1 else 0$
...
%This section writes to various files so the output to \verb|cmsyso.txt| must be redone afterwards.??
%
%%%%%see section ?? for essential code
...
rhs_:=rhsfn$
for k:=1:m do tmp_s(k):={for j:=1:n collect ee(j,k),cis(freq_(k)*t)};
for k:=1:m do tmp_z(k):={for j:=1:n collect zz(j,k),cis(freq_(k)*t)};
\end{reduce}
...
\begin{reduce}
%write rhsfn:=rhsfn;
%write manifolduu:=manifold_(uu_);
...
%For the moment, execute the example immediately.
%\begin{reduce}
%write "Executing the slow manifold example";
%exampleslowman();
%\end{reduce}
