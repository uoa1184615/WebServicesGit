...
-1+\rat32\varepsilon\cos^2 t & 1-\rat32\varepsilon\sin t\,\cos t\\
-1-\rat32\varepsilon\sin t\,\cos t&-1+\rat32\varepsilon\sin^2t
\end{bmatrix}.
\end{equation}
...
find instability predicted when \(\tfrac32\varepsilon
>1\); that is, \(|\varepsilon|>2/3\); for example,
...
\begin{align*}&
\dot s_{1}=\eps \big(3/4 s_{2}+3/4 s_{1}\big)+O\big(\varepsilon ^{9}
\big)
\\&
\dot s_{2}=\eps \big(3/4 s_{2}+3/4 s_{1}\big)+O\big(\varepsilon ^{9}\big)
\end{align*}
The eigenvalues of the above system are \(\lambda =0,
\tfrac32\varepsilon\).  Hence the net growth of~\uv\ is at
rate~\(-1+\tfrac32\varepsilon\); for example, at the
