...
Answer:  instead of using the dot product, we invoke the complex inner product that \(\left<\zv,\uv\right>:=\adj\zv\uv\) where the dagger denotes the complex-conjugate-transpose. 
That is, as \(a\C b\) are both real, here \(\adj\zv\)~is the row vector~\(\begin{bmatrix}1& a+\i b\end{bmatrix}\).
