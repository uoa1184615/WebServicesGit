...
variable~\(u_2(t-\pi)\), and where \verb|1+small^2*delta|
reflects that we wish to use the `small'
parameter~\(\delta\) to explore regimes where the coefficient is
near the value~\(1\).
\begin{reduce}
invariantmanifold({},
    mat(( -4*(1+small^2*delta)^2*(5/8*u2 +3/8*u2(pi)),
          +u1*(1+u2) )),
    mat(( i,-i,2*i,-2*i )),
...
\begin{align*}&
\dot u_{1}=-4(1+2\eps^2\delta+\eps^4\delta^2)\big[\tfrac58u_2+\tfrac38u_2(t-\pi)\big]
\\&
