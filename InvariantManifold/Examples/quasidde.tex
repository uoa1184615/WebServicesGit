...
We count order of nonlinearity using \(\verb|small|=\eps\), and count the parameter~\(\alpha\) as `small-squared' as is usual for Hopf bifurcations.
...
\begin{reduce}
invariantmanifold({},
    mat(( -4*u3-small^2*alpha*u3-small*2*u1^2-small^2*u1^3,
        2*u1-2*u2,
        2*u2-2*u3 )),
    mat((2*i,-2*i)),
