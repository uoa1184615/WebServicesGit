...
% upload a copy to WWW/GENCM, and 
...
%on ezgcd; 
%\end{reduce}
Let's make the algebra case sensitive: for \emph{csl} we set \verb|raise|; whereas for \emph{csl} we set \verb|lower|.  
...
\begin{reduce}
write "Construct invariant manifold  EXPERIMENTAL (version 21 Nov 2025)"$
if trace then <<
...
\end{reduce}
Set \verb|pdf| to be linear so it now expands.
Best to undo before exiting to ready for future execution.
\begin{reduce}
linear pdf;
\end{reduce}
...
write ll:=ll;
%
% remember pdfzero somewhere in here also??
write
uscale:=(for k:=1:n collect (mkid(u,k)=small_*mkid(u,k)));
write
ffs:=sub((small=small_).uscale,ff)/small_;
write
ffs:=sub(small_=small,ffs);
\end{reduce}
...
Need to clear all but operator properties of pdf.
\begin{reduce}
clear pdf; operator pdf;
