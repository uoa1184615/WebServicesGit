...
Sciences, Adelaide University, South Australia~5005,
\textsc{Australia}. \url{https://profajroberts.github.io/}}}

\date{Nov 2013 -- \today}
%:ToDo: change the '(version' date for each update, and
% upload a copy to WWW/GENCM, and WWW/Reduce, and
% upload a zipped copy of folder to WWW/Sites,

%:ToDo: currently the web service reduce chokes on du_k/dt=0 
% whereas my computer version does not.  The fix is to 
% specify du_k/dt=small^9 instead.  The error arises in 
% writing out the dynamical system to the file 
% invarManReportSys.tex ????

%:ToDo: change to matrices or arrays??
%:ToDo: modify LaTeX beginning to reduce spurious LaTeX
%:ToDo: use of exp clashes with 'on rounded' automatically 
% computing things like exp(0) is 1, so pattern exp(0)
% cannot be matched.  Maybe use exp_?

\usepackage{url,natbib,amsmath,defns,reducecode,etocx}
\RaisedNamesfalse
\let\harvardurl\url
...
invariantmanifold(spacex, odefns, evals, evecs, 
    adjvecs, toosmall);
\end{verbatim}
Output? as well as writing the result: \verb|uu| details 
the manifold;  \verb|gg|~details the evolution on the manifold; 
and if reported, \verb|zs| characterises the fast-fibre 
...
\verb|pdf()| operator (currently with at most five
arguments)---examples are \(\verb|pdf(u1,x)|=\D x{u_1}\),
\  \(\verb|pdf(u3,x,y)|=\Dx xy{u_3}\),
\  \(\verb|pdf(u2,x,4)|=\Dn x4{u_2}\).
...
dynamics by their evolution in time.  Generate a pdf report
...
\label{ss:eg}
Execute this example by invoking two commands: first,
\verb|in_tex "invariantManifold.tex"$| to load the procedure;
secondly \verb|exampleslowman();| to execute the example. 
The example system to analyse is specified to be 
\cite[Example~2.1]{Roberts2014a}
\begin{equation*}
...
\section{Start of the procedure code}
\localtableofcontents


Need a couple of things established before defining the
procedure: the \verb|rlfi| package, with later override 
of some  name translations.
\begin{reduce}
load_package rlfi; 
\end{reduce}
Allow `space' derivatives, to be assumed small, via this
\verb|pdf| operator that has the same syntax as \verb|df()|
but we need to use separate name in order to delay Reduce
processing:  \verb|pdf| defined by \verb|rlfi| package.
Change printing of all derivatives to partials (hacked 
from \verb|rlfi.red|).
\begin{reduce}
put('df,'lapop,"\partial ")$
\end{reduce}
Change printing of \verb|pdf()|, from \verb|rlfi.red|),
so it also caters for operators without an explicit function.
\begin{reduce}
% from reduce.rlfi, dated 23/05/1995
% AJR, this proc modified 17/3/2026
symbolic procedure dfprla(dfex,p)$
% Prints derivaves
begin
  scalar op,ord,arg,x,argup; 
  op:=p;%AJR, dummy to remove warning msg
  op:=get(car dfex,'lapop);
  % AJR, added to print operator if only one argument to p/df
  if null cddr dfex then <<         %AJR, case pdf(~x)
    arg:=" "; dfex:=cdr dfex; >>
  else if fixp caddr dfex then <<   %AJR, case pdf(~x,~n,...)
    arg:=" "; dfex:=cdr dfex; >>
  else << 
    arg:=cadr dfex;  
    dfex:=cddr dfex; 
...
a:if null cdr x then
  ...
  prin2la "\frac{";
  prin2la op;
  if ord=1 then %prin2la "\,"  % AJR, N/A omit
    else
      <<prin2la '!^!{;
        prin2la ord;
        prin2la "}" >>;
...
\end{reduce}
Define operator names for the variables 
of the dynamical system (in case they have 
...
Usually seems best to use the \verb|ezgcd| option---set here
in order for a user to optionally turn off before invoking
the procedure.  This option has mysteriously clashed fatally
with \verb|gcd| in some examples (Jul 2025), so appears best
without \verb|gcd|.  Unfortunately, \verb|ezgcd| crashes 
some \verb|psl| so only use \verb|csl| (Apr 2026).
\begin{reduce}
on ezgcd; 
\end{reduce}
Let's make the algebra case sensitive: for \emph{psl} we unset \verb|raise|; whereas for \emph{csl} we unset \verb|lower|.  
Remember (currently, 2025) \verb|off latex| automatically turns \verb|raise| back on (but not \verb|lower|).
\begin{reduce}
off raise,lower;
\end{reduce}
...
\subsection{Define the procedure as an operator} so we can define
...
\begin{reduce}
IMerrorMsg:="invariantmanifold() Should Have Six Arguments"$
for all a1,a2,a3,a4 let 
    invariantmanifold(a1,a2,a3,a4) = <<rederr(IMerrorMsg)>>;
for all a1,a2,a3,a4,a5 let 
    invariantmanifold(a1,a2,a3,a4,a5) = begin 
        write "**** Warning: ",IMerrorMsg;
        invariantmanifold({},a1,a2,a3,a4,a5);
        rederr(IMerrorMsg);
    end;
\end{reduce}
...
pp, est, eyem, m, pdfzero, ffnonpdf, ffpdf, ffnonsm,
ffsmall;
\end{reduce}

Write an intro message.
\begin{reduce}
write "Construct invariant manifold (version 28 Apr 2026)"$
%write input_odefns:=odefns;
%write input_evals:=evals;
%write input_evecs:=evecs;
%write input_adjvecs:=adjvecs;
%write input_toosmall:=toosmall;
\end{reduce}
...
ee := tp evecs;
zz := tp adjvecs; 
write "Input sizes: odes ",length(ff)
    ,"; evals ",length(evals)
    ,"; evecs ",length(ee)
    ,"; adjvs ",length(zz);
\end{reduce}
...
Define default parameters for the iteration: first,
...
Although seems OK for all other examples.
Get case sensitive analysis by also \verb|off raise,lower;|
\begin{reduce}
on div,revpri; off allfac;
on rationalize;
\end{reduce}
...
\end{reduce}
Also set rules when needed to omit \verb|pdf| terms,
currently coded up to five arguments.
\begin{reduce}
pdfzero:={ pdf(~a)=>0, pdf(~a,~b)=>0, pdf(~a,~b,~c)=>0
    , pdf(~a,~b,~c,~d)=>0, pdf(~a,~b,~c,~d,~e)=>0 };
\end{reduce}
...
But first test to see if there are any explicit time
dependence: whether autonomous or nonautonomous.
\begin{reduce}
tmp:=sub(t=t_,ff);  tmp:=df(tmp,t_);
itisAuto_:=1;
for j:=1:n do itisAuto_:=if tmp(j,1)=0 then itisAuto_ else 0;
...
conjugation.   Consequently, this procedure implicitly
assumes that all parameter and variable names are real,
except \(\i:=\sqrt{-1}\).
%: ToDo: is this adequate? does pdf() also need consideration??
\begin{reduce}
...
only works for pure imaginary modes!??   In cases where 
there are slow-space variations we need to commute some
derivatives one way, and commute other derivatives the
other way---all a bit tricksy and unclear enough factors
coded here.  
%: ToDo: needs checking
Also unclear is that \verb|df()| appears to 
order its arguments in alphabetical order (A--Z,a--z): 
so `space' variables before t-for-time may be in the
wrong position.   Maybe use \verb|pdf()| for space 
derivative working throughout, instead of \verb|df|??
\begin{reduce}
...
let { d_(~a^~p,t,~dt)=>d_(a,t,dt)^p
    ...
    , df(d_(~a,t,~dt),~b) => d_(df(a,b),t,dt) when not(b member spacex)
    , d_(df(~a,~b),t,~dt) => df(d_(a,t,dt),b) when (b member spacex)
    , d_(df(~a,~b,~c),t,~dt) => df(d_(a,t,dt),b,c) when (b member spacex)
    , d_(df(~a,~b,~c,~d),t,~dt) => df(d_(a,t,dt),b,c,d) when (b member spacex)
    , d_(~a,t,0)=>a
...
somerules:={}$
somerules:=(u1(~dt)=d_(u1,t,dt)).somerules$
somerules:=(u2(~dt)=d_(u2,t,dt)).somerules$
somerules:=(u3(~dt)=d_(u3,t,dt)).somerules$
somerules:=(u4(~dt)=d_(u4,t,dt)).somerules$
somerules:=(u5(~dt)=d_(u5,t,dt)).somerules$
somerules:=(u6(~dt)=d_(u6,t,dt)).somerules$
somerules:=(u7(~dt)=d_(u7,t,dt)).somerules$
somerules:=(u8(~dt)=d_(u8,t,dt)).somerules$
somerules:=(u9(~dt)=d_(u9,t,dt)).somerules$
...
for j:=1:n do for k:=1:n do begin 
  ...
  ll(j,k):=( sub(equilibrium,ll(j,k)) where pdfzero );
end;
...
Anything not in the linear operator, and not already 
multiplied by~\verb|small|, gets multiplied by
...
ffnonpdf:=(ff where pdfzero);
procedure scalepdf(z_);
    (z_ where {pdf(~a,~b)=>small*df(a,b) 
        , pdf(~a,~b,~n)=>small^n*df(a,b,n) when fixp(n)
        , pdf(~a,~b,~c)=>small^2*df(a,b,c) when not fixp(c)
        });
ffpdf:=map(scalepdf(~z),ff-ffnonpdf);
ffnonsm:=sub(small=0,ffnonpdf);
ffsmall:=ffnonpdf-ffnonsm;
ff:=small*ffnonsm+(1-small)*ll0_
    +ffsmall+ffpdf;
\end{reduce}
...
\begin{reduce}
itisAutoSlowMan_:=if eval_set={0} then itisAuto_ else 0$
if trace then write "itisAutoSlowMan_= ",itisAutoSlowMan_;
\end{reduce}
...
\begin{reduce}
procedure inv_proc(a_,invs);  begin scalar tmp_;
  if a_ member eval_set
  then << k_:=0; 
    repeat k_:=k_+1 until a_=eval_s(k_);
    tmp_:=invs(k_)*exp(a_*t) >>
  else tmp_:=sub(geneval=a_,invs(0))*exp(a_*t)$
  return tmp_;
  end;
\end{reduce}
...
Now that we have finished with \verb|pdf|, repurpose 
it for the bare differentiation operator needed for the 
projection vectors.  This seems to work for 1-D space PDEs, 
but needs checking.   
\begin{reduce}
if spacex neq {} then
let { df(~a,~x,~b)=>pdf(x) when a=b 
    , df(~a,~x,~n,~b)=>pdf(x)^n when a=b and fixp(n)
    , df(~a,~x,~y,~b)=>pdf(x)*pdf(y) 
        when a=b and not fixp(y)
    , df(~a,~x,~n,~y,~b)=>pdf(x)^n*pdf(y) 
        when a=b and fixp(n) and not fixp(y)
    , df(~a,~x,~y,~n,~b)=>pdf(x)*pdf(y)^n 
        when a=b and not fixp(y) and fixp(n)
    , df(~a,~x,~y,~z,~b)=>pdf(x)*pdf(y)*pdf(z) 
        when a=b and not fixp(y) and not fixp(z)
    }; 
\end{reduce}
%%The following fails to simplify pdf products for some unknown reason,
%%but it works if applied afterwards!  Why??
%let { df(~a,~x,~b)=>pdf(x,1) when a=b 
%    , df(~a,~x,~n,~b)=>pdf(x,n) when a=b and fixp(n)
%    , df(~a,~x,~y,~b)=>pdf(x,1,y,1) 
%        when a=b and not fixp(y)
%    , df(~a,~x,~n,~y,~b)=>pdf(x,n,y,1) 
%        when a=b and fixp(n) and not fixp(y)
%    , df(~a,~x,~y,~n,~b)=>pdf(x,1,y,n) 
%        when a=b and not fixp(y) and fixp(n)
%    , df(~a,~x,~y,~z,~b)=>pdf(x,1,y,1,z,1) 
%        when a=b and not fixp(y) and not fixp(z)
%    , pdf(~x,~n)^~p=>pdf(x,n*p) when fixp(n)
%    , pdf(~x,~n)*pdf(~y,~m)=>pdf(x,m+n) when x=y and fixp(n) and fixp(m)
%    , pdf(~x,~n)*pdf(~y,~m)=>pdf(x,m,y,n) when x neq y and fixp(n) and fixp(m)
%    }; 
...
let { df(s(~j),t)=>coeffn(gg,e_(j,1),1)
	, df(s(~j),t,~b)=> df(coeffn(gg,e_(j,1),1),b) when not fixp(b)
	, df(s(~j),t,~b)=> df(coeffn(gg,e_(j,1),1),t,b-1) when (fixp(b) and b>1)
	, df(s(~j),t,~b,~c)=> df(coeffn(gg,e_(j,1),1),b,c) when not fixp(b)
	, df(s(~j),t,~b,~c)=> df(coeffn(gg,e_(j,1),1),t,b-1,c) when (fixp(b) and b>1)
	, df(s(~j),t,~b,~c,~d)=> df(coeffn(gg,e_(j,1),1),b,c,d) when not fixp(b)
	, df(s(~j),t,~b,~c,~d)=> df(coeffn(gg,e_(j,1),1),t,b-1,c,d) when (fixp(b) and b>1)
	};
\end{reduce}
...
\begin{reduce}
if itisAutoSlowMan_ then begin
...
end %if itisAutoSlowMan_
else resd:=resz:=0; % for when not slow manifold
\end{reduce}
...
clearing~\verb|zs| when it is not computed.
\begin{reduce}
if {resde,resd,resz}={0,0,0} then write iter:=iter+10000;
end;
showtime;
if not itisAutoSlowMan_ then clear zs;
\end{reduce}
...
\section{Output text report of results}
...
\begin{reduce}
if itisAutoSlowMan_ then begin write "
...
%: ToDo.  There is an as yet unresolved problem in the typesetting
...
\section{Output LaTeX report of results}
...
First define how various tokens get printed.   Package
\verb|rlfi| must already be loaded.  
\begin{reduce}
deflist('((!( !\!b!i!g!() (!) !\!b!i!g!))
...
upon the command \verb|on latex|. Modified from
...
<<!*!*a2sfn:='texaeval;
  ...
  prin2t "\usepackage{parskip,time,verbatim} \def\_{_}"; 
  prin2t "\raggedright \def\eps{\varepsilon}";
  prin2t "\def\diff{\partial\makebox[0pt]{\ $\cdot$}}";
  ...
  prin2t "\par\leftskip=2em  \parindent=-2em";
...
\end{reduce}
%Override the procedure that ends \LaTeX\ output, assumes always preceded by an \verb|on latex|.
%\begin{reduce}
